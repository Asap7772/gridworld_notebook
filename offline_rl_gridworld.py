# -*- coding: utf-8 -*-
"""Offline Rl Gridworld

Automatically generated by Colaboratory.

Original file is located at
		https://colab.research.google.com/drive/1SqtnuJ-7H_iYyZyctZ4gN5olOehCbp1s
...
parser.add_argument('--const_transform', type=float, default=4)
    ...
    **kwargs):
    ...
    for i in range(num_itrs):
        if sampled:
            ...
            target_values = q_backup_sparse(env, q_values, **kwargs)
            q_values = project_qvalues_cql(target_values, network, optimizer,weights=weights_tensor, cql_alpha=cql_alpha, num_steps=project_steps)
        if render:
            plot_sa_values(env, q_values, update=True, title='Q-values Iteration ' + str(i).zfill(3), prefix=prefix)
    return q_values
...
elif dataset_composition == 'segment':
...
# Run Q-iteration
q_values = fitted_q_iteration(env, network, num_itrs=50, discount=0.95, weights=weights, render=True, sampled=False, prefix='FullQ/')

# Compute and plot the value function
v_values = np.max(q_values, axis=1)
plot_s_values(env, v_values, title='Values', prefix='FullQ/')
...
plot_sa_values(env, (q_values - optimal_qvalues), title='Q-function Error (Q - Q*)', prefix='FullQ/')

#@title Compute visitations of the learned policy
policy = compute_policy_deterministic(q_values, eps_greedy=0.1)
policy_sa_visitations = compute_visitation(env, policy)
plot_sa_values(env, policy_sa_visitations, title='Q-hat Visitation', prefix='FullQ/')
...
# Run Q-iteration
q_values = conservative_q_iteration(env, network, num_itrs=50, discount=0.95, cql_alpha=cql_alpha_val, weights=weights, render=True, transform_type=transform_type, const_transform=const_transform, prefix='FullCQL/')
...
# Run Q-iteration
q_values = fitted_q_iteration(env, network, num_itrs=100, discount=0.95, weights=weights, render=True, sampled=not(weighting_only), training_dataset=training_dataset, prefix='finiteQ/')

# Compute and plot the value function
v_values = np.max(q_values, axis=1)
plot_s_values(env, v_values, title='Values', prefix='finiteQ/')
...
plot_sa_values(env, (q_values - optimal_qvalues), title='Q-function Error (Q - Q*)', prefix='finiteQ/')

#@title Compute visitations of the learned policy
policy = compute_policy_deterministic(q_values, eps_greedy=0.1)
policy_sa_visitations = compute_visitation(env, policy)
plot_sa_values(env, policy_sa_visitations, title='Q-hat Visitation', prefix='finiteQ/')
...
# Run Q-iteration
q_values = conservative_q_iteration(env, network, num_itrs=100, discount=0.95, cql_alpha=cql_alpha_val, weights=weights, render=True, sampled=not(weighting_only), training_dataset=training_dataset, transform_type=transform_type, const_transform=const_transform, prefix='finiteCQL/')
