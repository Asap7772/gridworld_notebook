...
def project_qvalues_sampled(env, s, a, optimal_q_values, network, optimizer, temperature=1, num_steps=50, weights=None):
    # train with a sampled dataset
    s = torch.tensor(s, dtype=torch.int64)
    a = torch.tensor(a, dtype=torch.int64)
    
    curr_q_optimal = optimal_q_values.gather(0, s.reshape(-1,1)).gather(1, a.reshape(-1,1))
    values_optimal = optimal_q_values.gather(0, s.reshape(-1,1)).mean(1, keepdims=True)
    advantages = curr_q_optimal - values_optimal
    
    logits = network(s)
    logits = logits.gather(1, a.reshape(-1,1)).squeeze()

    log_probs = F.log_softmax(logits, dim=-1)

    loss = -log_probs * torch.exp(advantages/temperature)
    loss = loss.mean()
        ...
        if sampled:
            for j in range(project_steps):
                ...
                intermed_values = project_qvalues_sampled(
                                env, s, a, optimal_q_values, network, optimizer, weights=None,
                )
...
# Run Q-iteration
q_values = awr_iteration(env, network, num_itrs=num_itrs, discount=discount, cql_alpha=cql_alpha_val, weights=weights, render=args.render, sampled=not(weighting_only), training_dataset=training_dataset, transform_type=transform_type, const_transform=const_transform, prefix='finiteCQL/', optimal_policy=optimal_policy, optimal_q_values=optimal_q_values)
